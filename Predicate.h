...
private:
	string _name;
	vector<string> _list;
	vector<Parameter> parameterList;  // FIXME -> this is no longer a pointer, need to get rid of child classes
...
public:
	...
	bool setPredicateFact(unsigned int& tracker, vector<Token> tokenList, bool& pass) {
		...
		if ((&tokenList.at(tracker) != NULL) && tokenList.at(tracker).getTokenCode() == 14) { // Checks for STRING
			_list.push_back(tokenList.at(tracker).getTokenValue());
			++tracker;
			//continue;
		}
